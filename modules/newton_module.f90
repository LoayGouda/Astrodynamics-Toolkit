...
!  Newton's method for root finding of scalar function f(x)
    ...
    real(kind=8),intent(in)    :: x         !! initial point (initial guess)
    procedure(func)        :: f         !! function f(x)
    procedure(func)        :: dfdx      !! first derivative function f'(x)
    real(kind=8),intent(in)    :: ftol      !! convergence tolerance for f(x)
    real(kind=8),intent(in)    :: xtol      !! convergence tolerance for x
    integer,intent(in)     :: max_iter  !! the maximum number of iterations
    real(kind=8),intent(out)   :: xs        !! the value where f(x) is zero
    real(kind=8),intent(out)   :: fx        !! the value of f(x) at the root xs
    ...
    real(kind=8)  :: f1,df1,x1,x1_prev
    integer   :: iter
    real(kind=8),parameter :: alpha = one !! step factor
